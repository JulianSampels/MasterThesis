...
class BaseCandidateGenerator(ABC):
    ...
    def _compute_adaptive_head_block_size(self, E: int, k_total: int) -> int:
        ...
        memory_budget_bytes = int(2.0e8)  # 200MB per worker (adjust based on your RAM/GPU)
        bytes_per_float = 4
        bytes_per_long = 8
...
class CandidateGeneratorGlobal(BaseCandidateGenerator):
    ...
    def _global_topk_joint_streaming(
        ...
        if k_total > 500 * 14000 and num_workers > 100:
            num_workers = min(num_workers, self.max_num_workers // 4) # Limit workers for very large k_total to avoid OOM
