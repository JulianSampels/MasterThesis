...
# ----------------------------------------------------------------------------- #
# 1. Geometry & Font Configuration for LaTeX Matching
# ----------------------------------------------------------------------------- #
# Standard scrreprt text width is ~15.0 cm (approx 5.9 inches)
TEXT_WIDTH_CM = 15.0  
TEXT_WIDTH_INCH = TEXT_WIDTH_CM / 2.54

# Standard font size for the document
FONT_SIZE_PT = 11
...
plt.rcParams.update({
    ...
    # Set sizes to match document exactly
    "font.size": FONT_SIZE_PT,       
    "axes.labelsize": FONT_SIZE_PT,  
    "axes.titlesize": FONT_SIZE_PT +1,
    "xtick.labelsize": FONT_SIZE_PT - 2, # Slightly smaller ticks
...
def plot_grouped_hist(ax, hist_list, title, xlabel, linewidth):
    ...
    for i, hist in enumerate(hist_list):
        ...
        ax.bar(
            ...
            alpha=1.0,
            label=display_names[datasets[i]],
            edgecolor="black",
            linewidth=linewidth,
            zorder=2,
        )
    ...
    ax.set_title(title)
    ax.set_xlabel(xlabel)
    ax.set_ylabel("Freq. (log)") # Shortened label to save space
    ax.set_yscale("log")
...
for dataset in datasets:
    ...
    triples = triples[
        torch.isin(
            triples[:, 1],
            torch.tensor(list(train_rel2inv.keys()), dtype=torch.long, device=triples.device),
        )
    ]
    triples = triples.cpu()
    ...
    tails = triples[:, 2]
    rels = triples[:, 1]
    all_tails = torch.unique(tails)
    all_heads = torch.unique(heads)
    all_rels = torch.unique(rels)
    ...
    data[dataset] = {
        "hist_tail_per_head": hist_tail_per_head,
        "hist_head_per_tail": hist_head_per_tail,
        "hist_rel_per_head": hist_rel_per_head,
        "hist_rel_per_tail": hist_rel_per_tail,
    }
...
# Height calculation: 4 rows on a page usually fit well in approx 22-24cm 
# We set roughly 9 inches height for good vertical spacing.
HEIGHT_INCH = 7.9

fig, axes = plt.subplots(4, 2, figsize=(TEXT_WIDTH_INCH, HEIGHT_INCH), sharey="row")

# Row 0: Combined (Linewidth 0.8)
plot_grouped_hist(
    axes[0, 0],
    hist_tail_combined,
    "Tail Multiplicity per Head", # Shortened title
    "Relations per (Head, Tail) Pair",
    linewidth=0.7
)
plot_grouped_hist(
    axes[0, 1],
    hist_head_combined,
    "Head Multiplicity per Tail", # Shortened title
    "Relations per (Tail, Head) Pair",
    linewidth=0.7
)
axes[0, 1].set_ylabel("")
axes[0, 1].legend(loc="upper right", frameon=True)

# Rows 1–3: Per-dataset
for i, dataset in enumerate(datasets):
    row = i + 1
    color = colors_datasets[i]
    name = display_names[dataset]
    
    # Specific Linewidth Logic
    ...
    if dataset == "jf17k":
        plot_separated_bar(
            axes[row, 0],
            data[dataset]["hist_rel_per_head"],
            f"{name}: Rel. Multiplicity per Head",
            "Tails per (Head, Relation) Pair",
            color,
            linewidth=current_lw
        )
    else:
        plot_stepfilled_hist(
            axes[row, 0],
            data[dataset]["hist_rel_per_head"],
            f"{name}: Rel. Multiplicity per Head",
            "Tails per (Head, Relation) Pair",
    ...
    plot_stepfilled_hist(
        axes[row, 1],
        data[dataset]["hist_rel_per_tail"],
        f"{name}: Rel. Multiplicity per Tail",
        "Heads per (Tail, Relation) Pair",
        color,
        linewidth=current_lw
    )
    axes[row, 1].set_ylabel("")

# Use constrained_layout for optimal packing within the small geometry
fig.set_constrained_layout(True)
fig.get_layout_engine().set(h_pad=0.05, w_pad=0) # Increase h_pad for more vertical space

os.makedirs("./figures", exist_ok=True)
plt.savefig("./figures/combined_occurrence_figure.svg")
plt.close()

print(f"Figure saved to ./figures/combined_occurrence_figure.svg ({TEXT_WIDTH_INCH:.2f}x{HEIGHT_INCH} inches)")