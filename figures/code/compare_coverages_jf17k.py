...
def create_compare_coverage_vs_avg_group_size_plot(csv_paths, labels=None, save_dir="./figures", filename="compare_coverage_vs_avg_group_size.pgf", relative_divisor=None):
    ...
    for i, (df, label_parts) in enumerate(zip(dfs, labels)):
        l, = ax1.plot(df['candidate_size'], df['avg_cov_per_group'], 
                      marker='o', markersize=3, linestyle='-', color=colors[i % len(colors)])
        lines_cov.append(l)
    ...
    for i, (df, label_parts) in enumerate(zip(dfs, labels)):
        l, = ax2.plot(df['candidate_size'], df['avg_group_density'], 
                      marker='x', markersize=3, linestyle=':', color=colors[i % len(colors)])
        lines_den.append(l)
    ...
    rect = Rectangle(
        (1.02e6, 0.64),      # (x, y) Bottom-Left corner
        4.345e6,                # width
        0.35,               # height
...
if __name__ == "__main__":
    ...
    csv_paths = [
        "./logs/Final/jf17k/pathe2Phases/RelationBceTailBce/version_0/figures/coverage_vs_size_results.csv",
        "./logs/Final/jf17k/pathe2Phases/RelationBceTailHurdle/version_0/figures/coverage_vs_size_results.csv",
        "./logs/Final/jf17k/pathe2Phases/RelationBceTailP/version_0/figures/coverage_vs_size_results.csv",
        "./logs/Final/jf17k/pathe2Phases/RelationHurdleTailHurdle/version_0/figures/coverage_vs_size_results.csv",
        "./logs/Final/jf17k/pathe2Phases/RelationNBTailNB/version_0/figures/coverage_vs_size_results.csv",
        "./logs/Final/jf17k/pathe2Phases/RelationPTailBce/version_0/figures/coverage_vs_size_results.csv",
        "./logs/Final/jf17k/pathe2Phases/RelationPTailHurdle/version_0/figures/coverage_vs_size_results.csv",
        "./logs/Final/jf17k/pathe2Phases/RelationPTailP/version_0/figures/coverage_vs_size_results.csv",
    ]
    labels = [
        ("BCE", "BCE"), ("BCE", "Hurdle"), ("BCE", "Poisson"),
        ("Hurdle", "Hurdle"), ("NB", "NB"),
        ("Poisson", "BCE"), ("Poisson", "Hurdle"), ("Poisson", "Poisson")
    ]
    create_compare_coverage_vs_avg_group_size_plot(csv_paths, labels, filename="jf17k_compare_macro.pgf", relative_divisor=6823)